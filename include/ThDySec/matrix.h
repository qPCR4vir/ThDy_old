...
public:
	...
	RowType Row(index row){ assert(row<totalRow() ); return _mtx[row];}
...
{   if(_capRow<=totalRow() && _capCol<=totalCol() )  return ;
	...
	swap(mtx);
		...
		template <typename Num>		//			copia contenido, aumentando zona en uso y cap si necesario. Conserva datos propios no reescritos
